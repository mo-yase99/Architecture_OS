...
export async function POST(req: Request) {
  ...
  const checklist = ['Review today’s highest-priority task','Check overdue/due-today tasks','Review drawing/revision changes','Check open issues and RFIs','Check procurement due dates','Record executed quantities with evidence','Update progress and cost records','Finish active file-intelligence jobs','Prepare tomorrow lookahead and blockers']
  const urgent = [
  ...
  const { data, error } = await s.from('ai_recommendations').insert({ user_id: user.id, project_id: projectId, recommendation_type: 'daily_site_assistant', title: `Daily engineering assistant — ${day}`, recommendation, priority: urgent.length ? 'high' : 'medium', confidence: .9, action: nextAction, source_refs: { date: day, checklist, urgent, context: { tasks: tasks?.length || 0, issues: issues?.length || 0, rfis: rfis?.length || 0, procurement: procurement?.length || 0, active_jobs: jobs?.length || 0, recent_progress: progress?.length || 0 } }, status: 'new' }).select().single()
