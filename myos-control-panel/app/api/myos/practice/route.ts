...
export async function POST(request: Request) {
  try {
    ...
    const candidates = chosen.practice.map((task, index) => ({ id:`${chosen.id}-${index+1}`, title:task, skill:chosen.name, roadmapStage:chosen.stage, estimatedMinutes:index===0?Math.min(minutes,30):Math.min(minutes,45), reason:index===0?'Highest-value practice for your current roadmap stage.':'Reinforces the skill through a practical deliverable.' }));
    const selected = candidates[(completedTaskTitles.length + learnedSkillIds.length) % candidates.length];
    return NextResponse.json({ mode:'adaptive-v1', recommendation:selected, skill:chosen, analysis:{minutes,energy,consideredSkills:scored.slice(0,5).map(x=>({id:x.skill.id,name:x.skill.name,score:x.score})), learnedSkillIds, recentTaskAreas, completedTaskCount:completedTaskTitles.length} });
