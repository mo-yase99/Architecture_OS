...
export async function GET() {
  ...
  if (!token) return NextResponse.json({ mode: 'demo', tasks: [], message: 'Add NOTION_TOKEN in Vercel Environment Variables.' });
  try {
    ...
    if (!response.ok) return NextResponse.json({ mode: 'error', tasks: [], detail: raw }, { status: response.status });
    const data = JSON.parse(raw);
    const tasks = (data.results as NotionPage[]).map(page => {
      const p = page.properties;
      const title = Object.values(p).find((x: any) => x.type === 'title');
      ...
      const status = Object.values(p).find((x: any) => ['status','select'].includes(x.type) && ['Not started','In progress','Done','Completed'].includes(x.status?.name ?? x.select?.name));
      return { id: page.id, title: textFromProperty(title), area: textFromProperty(area), priority: textFromProperty(priority), minutes: numberFromProperty(minutes), status: textFromProperty(status) };
    });
